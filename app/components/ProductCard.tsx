...
const ProductCard: React.FC<ProductData> = ({ product }) => {
     return (
          <Link key={product.id} href={`detail/${product.id}`}>
               <section className='border-solid border-2 border-sky-500 rounded-md w-80 h-80 p-2 cursor-pointer'>
                    <Image className='object-cover' width={320} style={{ height: '80px' }} height={0} alt={product.title} src={product.images[0]} />
